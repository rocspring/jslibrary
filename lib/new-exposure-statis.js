 ...
 * @version: 0.0.4 (2015.12.02)
 * 1. 添加了一个判断页面是向上滑还是向下滑的属性
 * 2. 重新实现判断元素曝光的方法，使得向上滑动和向下滑动都可以统计曝光
 * @version: 0.0.5 (2015.12.10) callback方法默认传入了一个滑动方向的参数slideDirection
 * @version: 0.0.6 (2016.1.13) 增加一个可以外部传入的判断元素是否曝光的方法()
 * @version: 0.0.7 (2016.3.15) 一个元素曝光过后，清除它的判断曝光的属性，使得可以判断它的再次曝光
 */
...
(function(window) {
		...
		remove: function(arr, index) {
			if (typeof arr === 'object' && toString.call(arr) === '[object Array]') {
				if (typeof index === 'number' && index >= 0) {
					return arr.remove ? arr.remove(index) : arr.slice(0, index).concat(arr.slice(
						index + 1, arr.length));
				} else if (typeof index === 'object' && toString.call(index) ===
					'[object Array]') {
					//传入一个下标数组，删除这些元素
					var newArr = [];
	...
	NewExposureStatis.prototype = {
		...
		addNewElements: function(inputParameter) {
			...
			// 传入的是dom数组时
			if (toString.call(inputParameter) === '[object Array]' && inputParameter.length !==
				0) {
			...
			} else if (toString.call(inputParameter) === '[object Object]' && !!
				inputParameter.dom) {
		...
		addExposureListen: function() {
			...
			window.addEventListener('scroll', utils.proxy(that.exposureListenFunc,
				that), false);
		},
		...
		removeExposureListen: function() {
			...
			window.removeEventListener('scroll', utils.proxy(that.exposureListenFunc,
				that), false);
		},
		...
		allIsExposure: function(arr) {
			...
			for (i = 0; i < len; i++) {
				// isExpos = this.isExposure({
				// 	dom: domArr[i].dom,
				// 	overHeight: domArr[i].overHeight,
				// 	otherJudgeMethod: domArr[i].otherJudgeMethod
				// });
				isExpos = this.isExposure(domArr[i].dom, domArr[i].overHeight, domArr[i]
					.otherJudgeMethod);
		...
		isExposure: function() {
			...
			if (domObj) {
				...
				// 判断元素的底部是否暴露在屏幕中
				if (distanceScreenTopHeight > -distance && distanceScreenTopHeight <
